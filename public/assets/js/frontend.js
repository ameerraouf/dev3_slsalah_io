( () => {
	...
	document.addEventListener( 'click', ev => {
		...
		if (
			...
			(
				!siteNavContainer?.contains( clickedEl ) && !siteNavContainer !== clickedEl
			) &&
			(
				!mobileNavTrigger?.contains( clickedEl ) && !mobileNavTrigger !== clickedEl
			)
		) {
			mobileNavTrigger?.classList.remove( 'lqd-is-active' );
			siteNavContainer?.classList.remove( 'lqd-is-active' );
		...
		if ( clipboardCopyButton ) {
			const settings = JSON.parse( clipboardCopyButton.getAttribute( 'data-copy-options' ) || {} );
			let getContentFrom;

			if ( settings.contentIn ) {
				if ( settings.contentIn.startsWith( '<' ) && settings.content ) {
					const el = clipboardCopyButton.parentElement.closest( settings.contentIn.replace( '<', '' ) );
					getContentFrom = el.querySelector( settings.content );
				}
			} else {
				getContentFrom = settings.content ? document.querySelector( settings.content ) : clipboardCopyButton.parentElement
			}
