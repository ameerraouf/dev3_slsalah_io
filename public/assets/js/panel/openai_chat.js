...
$(document).ready(function () {
	...
	function updateChatsByFilter() {
		$(".chat_element").each(function () {
			...
			let description = $(this.querySelector('.chat_description')).text().toLowerCase();
			if ((chatFilter == "All" || chatFilter == $(this).attr('category') || (isFav != 0 && chatFilter == "Favorite")) && (name.includes(serachFilter) || description.includes(serachFilter))) {
				$(this).removeClass('hidden');
			} else {
				$(this).addClass('hidden');
			}
...
(($) => {
	...
	function submitForm(ev) {
		...
		if (!constvalue || constvalue.length === 0 || constvalue.replace(/\s/g, '') === '') {
		...
		$.ajax({
			...
			success: function (data) {
				...
				userBubbleTemplate.querySelector('.chat-content').innerHTML = constvalue;
				...
				function implementChat(type, images) {
					...
					const eventSource = new EventSource("/dashboard/user/openai/chat/chat-send?chat_id=" + chat_id + "&message_id=" + message_id + "&type=" + type + "&images=" + images + "&pdfname=" + pdfName + "&pdfpath=" + pdfPath);
